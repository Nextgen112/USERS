    ...
    if (!D.getElementById("nexus-styles")) { 
        var CSS = `
            .wcbg {
                ...
                background: rgba(6, 8, 14, 0.75); backdrop-filter: blur(8px); 
                z-index: 99999; display: none; align-items: center; justify-content: center; 
                font-family: system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;
                transition: opacity 0.25s ease-out;
            }
            .wcpanel {
                background: rgba(13, 17, 26, 0.95); border: 1px solid rgba(0, 210, 255, 0.25); 
                box-shadow: 0 20px 50px rgba(0,0,0,0.6), 0 0 20px rgba(0, 210, 255, 0.1);
                border-radius: 16px; width: 90%; max-width: 450px; max-height: 85vh; 
                overflow-y: auto; padding: 24px; color: #f0f4f8; position: relative;
                scrollbar-width: thin; scrollbar-color: rgba(0, 210, 255, 0.2) transparent;
            }
            .wctitle {
                background: linear-gradient(90deg, #00f2fe, #4facfe); -webkit-background-clip: text; 
                -webkit-text-fill-color: transparent; text-align: center; font-size: 16px; 
                font-weight: 800; margin-bottom: 20px; letter-spacing: 5px; text-transform: uppercase;
            }
            .wctabs {
                display: flex; gap: 6px; margin-bottom: 18px; background: rgba(0, 0, 0, 0.3); 
                padding: 5px; border-radius: 10px; overflow-x: auto;
            }
            .wctab {
                flex: 1; padding: 8px 6px; background: transparent; border: none; color: #74859a; 
                cursor: pointer; font-size: 11px; font-weight: 700; border-radius: 6px; 
                text-transform: uppercase; text-align: center; min-width: 75px; transition: all 0.2s ease;
            }
            .wctab:hover { color: #00f2fe; background: rgba(0, 210, 255, 0.08); }
            .wctab.on { background: linear-gradient(135deg, #4facfe, #00f2fe); color: #07090e; font-weight: 800; box-shadow: 0 2px 8px rgba(0, 210, 255, 0.3); }
            .wcsec { display: none; }
            .wcsec.on { display: block; animation: fadeIn 0.2s ease-out; }
            .wcgrid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 10px; margin-bottom: 16px; }
            .wcbtn {
                padding: 11px 10px; background: rgba(255, 255, 255, 0.03); border: 1px solid rgba(255, 255, 255, 0.06); 
                color: #e2e8f0; cursor: pointer; font-size: 11.5px; font-weight: 600; border-radius: 8px; 
                text-transform: capitalize; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; 
                transition: all 0.15s ease;
            }
            .wcbtn:hover { background: rgba(0, 210, 255, 0.12); color: #00f2fe; border-color: rgba(0, 210, 255, 0.4); transform: translateY(-1px); }
            .wcbtn:active { transform: translateY(0); }
            .wcbtn.buying { opacity: .3; pointer-events: none; }
            .wclbl { color: #475569; font-size: 10px; margin: 16px 0 8px; text-transform: uppercase; letter-spacing: 2px; font-weight: 700; display: flex; align-items: center; gap: 8px; }
            .wclbl::after { content: ''; flex: 1; height: 1px; background: rgba(255, 255, 255, 0.06); }
            .wcclose {
                position: absolute; top: 20px; right: 20px; color: #64748b; font-size: 14px; 
                cursor: pointer; width: 26px; height: 26px; display: flex; align-items: center; 
                justify-content: center; border-radius: 50%; background: rgba(255, 255, 255, 0.03); transition: all 0.2s;
            }
            .wcclose:hover { color: #ff4b72; background: rgba(255, 75, 114, 0.15); transform: rotate(90deg); }
            .wclog {
                max-height: 95px; overflow-y: auto; font-size: 11px; color: #94a3b8; background: rgba(0, 0, 0, 0.4); 
                padding: 12px; border-radius: 10px; margin-top: 18px; border: 1px solid rgba(255, 255, 255, 0.03);
            }
            .wclog div { margin: 4px 0; padding-bottom: 4px; border-bottom: 1px solid rgba(255, 255, 255, 0.02); display: flex; align-items: center; gap: 6px; }
            .wclog .ok { color: #2ecc71; font-weight: 600; }
            .wclog .err { color: #ff4b72; font-weight: 600; }
            .wcbuyall {
                display: block; width: 100%; padding: 12px; margin: 12px 0 4px; background: rgba(0, 242, 254, 0.06); 
                color: #00f2fe; border: 1px solid rgba(0, 210, 255, 0.2); cursor: pointer; font-weight: 700; 
                text-transform: uppercase; font-size: 11px; border-radius: 8px; transition: all 0.2s ease;
            }
            .wcbuyall:hover { background: linear-gradient(90deg, #4facfe, #00f2fe); color: #07090e; box-shadow: 0 4px 12px rgba(0, 210, 255, 0.2); }
            .search-bar { display: flex; gap: 8px; margin-bottom: 18px; padding: 4px; background: rgba(0, 0, 0, 0.25); border-radius: 8px; border: 1px solid rgba(255,255,255,0.03); }
            .search-bar input { flex: 1; background: transparent; border: none; padding: 8px 12px; color: #fff; font-size: 12px; outline: none; }
            .search-bar button { background: rgba(255, 255, 255, 0.04); border: 1px solid rgba(255, 255, 255, 0.06); border-radius: 6px; color: #00f2fe; padding: 0 18px; cursor: pointer; font-weight: 700; font-size: 11px; transition: all 0.15s; }
            .search-bar button:hover { background: rgba(0, 210, 255, 0.15); border-color: rgba(0, 210, 255, 0.3); }
